...
this.checked);if(e.shiftKey&&lastChecked&&lastChecked!=e.target)Drupal.tableSelectRange($(e.target).closest("tr")[0],$(lastChecked).closest("tr")[0],e.target.checked);updateSelectAll(checkboxes.length==$(checkboxes).filter(":checked").length);lastChecked=e.target});updateSelectAll(checkboxes.length==$(checkboxes).filter(":checked").length)};Drupal.tableSelectRange=function(from,to,state){var mode=from.rowIndex>to.rowIndex?"previousSibling":"nextSibling";for(var i=from[mode];i;i=i[mode]){if(i.nodeType!=
1)continue;$(i).toggleClass("selected",state);$("input:checkbox",i).each(function(){this.checked=state});if(to.nodeType){if(i==to)break}else if($.filter(to,[i]).r.length)break}}})(jQuery);
...
function(context,settings,$adminMenu){if(settings.admin_menu.destination)$("a.admin-menu-destination",$adminMenu).each(function(){this.search+=(!this.search.length?"?":"&")+Drupal.settings.admin_menu.destination})};Drupal.admin.behaviors.hover=function(context,settings,$adminMenu){$("li.expandable",$adminMenu).hover(function(){clearTimeout(this.sfTimer);$("> ul",this).css({left:"auto",display:"block"}).parent().siblings("li").children("ul").css({left:"-999em",display:"none"})},function(){var uls=
$("> ul",this);this.sfTimer=setTimeout(function(){uls.css({left:"-999em",display:"none"})},400)})};Drupal.admin.behaviors.search=function(context,settings,$adminMenu){var $input=$("input.admin-menu-search",$adminMenu);var needle=$input.val();var links;var needleMinLength=2;var $results=$("<div />").insertAfter($input);function keyupHandler(){var matches,$html,value=$(this).val();if(value!==needle){needle=value;if(!links&&needle.length>=needleMinLength)links=buildSearchIndex($adminMenu.find("li:not(.admin-menu-action, .admin-menu-action li) > a"));
if(needle.length<needleMinLength)$results.empty();if(needle.length>=needleMinLength&&links){matches=findMatches(needle,links);$html=buildResultsList(matches);$results.empty().append($html)}}}function buildSearchIndex($links){return $links.map(function(){var text=this.textContent||this.innerText;if(typeof text==="undefined")return;return{text:text,textMatch:text.toLowerCase(),element:this}})}function findMatches(needle,links){var needleMatch=needle.toLowerCase();return $.grep(links,function(link){return link.textMatch.indexOf(needleMatch)!==
-1})}function buildResultsList(matches){var $html=$('<ul class="dropdown admin-menu-search-results" />');$.each(matches,function(){var result=this.text;var $element=$(this.element);var $category=$element.closest("#admin-menu-wrapper > ul > li");var categoryText=$category.find("> a").text();if($category.length&&categoryText)result=categoryText+": "+result;var $result=$('<li><a href="'+$element.attr("href")+'">'+result+"</a></li>");$result.data("original-link",$(this.element).parent());$html.append($result)});
return $html}function resultsHandler(e){var $this=$(this);var show=e.type==="mouseenter"||e.type==="focusin";$this.trigger(show?"showPath":"hidePath",[this])}function resultsClickHandler(e,link){var $original=$(this).data("original-link");$original.trigger("mouseleave");$input.val("").trigger("keyup")}function highlightPathHandler(e,link){if(link){var $original=$(link).data("original-link");var show=e.type==="showPath";$original.toggleClass("highlight",show);$original.trigger(show?"mouseenter":"mouseleave")}}
$results.delegate("li","mouseenter mouseleave focus blur",resultsHandler);$results.delegate("li","click",resultsClickHandler);$adminMenu.delegate(".admin-menu-search-results li","showPath hidePath",highlightPathHandler);$input.bind("keyup search",keyupHandler)}})(jQuery);